...
import {
  ...
  Copy, ExternalLink, Star, Clock, BarChart3
} from 'lucide-react'

function normalizeList(obj) {
  const candidate =
    (obj && obj.data && Array.isArray(obj.data.items) && obj.data.items) ||
    (obj && obj.data && Array.isArray(obj.data) && obj.data) ||
  ...
  useEffect(() => {
    ...
    async function loadInitial() {
      try {
        ...
        if (sRes.ok) setStores(normalizeList(sData))
  ...
  useEffect(() => {
    ...
    async function loadOffers() {
      try {
        ...
        if (oRes.ok) setOffers(normalizeList(oData))
        else setOffers([])
      } catch {
        setOffers([])
    ...
    <main className="min-h-screen mt-16 bg-gradient-to-b from-gray-50 to-white">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4 mb-6">
                <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 min-w-[140px]">
                  ...
                  <div className="text-2xl font-bold">₹{quickStats.totalEarnings.toLocaleString()}</div>
                </div>
                <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 min-w-[140px]">
                  <div className="text-sm text-blue-100">Pending</div>
                  <div className="text-2xl font-bold">₹{quickStats.pendingPayouts.toLocaleString()}</div>
                </div>
                <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 min-w-[140px]">
                  <div className="text-sm text-blue-100">Active Links</div>
                  <div className="text-2xl font-bold">{quickStats.activeLinks}</div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                {quickActions.map((action, index) => (
                  <a
                    key={index}
                    href={action.href}
                    className="px-6 py-3 bg-white text-blue-600 font-semibold rounded-xl hover:shadow-lg transition-all flex items-center gap-2"
                  >
                    {action.icon}
                    {action.label}
                  </a>
      ...
      <section className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 mb-3">
              <Sparkles className="w-4 h-4 text-amber-600" />
              <span className="text-sm font-semibold text-amber-700">HOT DEALS</span>
            </div>
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Featured Deals</h2>
            <p className="text-gray-600 mt-1">High commission offers for maximum earnings</p>
          </div>
          <a 
            href="/offers" 
            className="mt-4 md:mt-0 text-blue-600 hover:text-blue-700 font-semibold flex items-center gap-1"
          >
            View all offers
            ...
            {offers.map((offer, idx) => (
              <div key={offer._id || idx} className="group bg-white border border-gray-200 rounded-xl p-5 hover:shadow-xl hover:border-blue-300 transition-all duration-300">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="font-bold text-gray-900 line-clamp-1">{offer.title || offer.name || 'Offer'}</div>
                    </div>
                  </div>
                </div>
                  ...
                  {offer.rate ? (
                    <div className="inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-green-50 to-emerald-50 rounded-full">
                      ...
                      <span className="text-sm font-bold text-green-600">
                        Up to {Number(offer.rate).toFixed(0)}% commission
                      </span>
                    </div>
                  ) : null}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button 
                    onClick={() => generateCuelinksProduct(offer._id)}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-3 mt-4 md:mt-0">
              <div className="relative">
                ...
                <select 
                  className="pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  value={storeId} 
              ...
              {products.map(p => (
                <div key={p._id || p.id} className="group border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl hover:border-blue-300 transition-all duration-300">
                  ...
                  <div className="relative h-48 overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200">
                    ...
                    {/* Store Badge */}
                    <div className="absolute top-3 left-3 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-full text-xs font-semibold">
                      {p.store?.name || 'Store'}
                    </div>
